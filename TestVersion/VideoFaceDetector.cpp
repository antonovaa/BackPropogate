...
void VideoFaceDetector::detectFaceAllSizes(const  cv::Mat &frame)
{
	...
	// Maximum face size is 2/3rds of screen height// todo CASCADE_SCALE_IMAGE
	m_faceCascade->detectMultiScale(frame, m_allFaces, 1.1, 5, CASCADE_DO_CANNY_PRUNING,
...
void VideoFaceDetector::detectFaceAroundRoi(const  cv::Mat &frame)
{
	...
	m_faceCascade->detectMultiScale(frame(m_faceRoi), m_allFaces, 1.1, 5, CASCADE_DO_CANNY_PRUNING,
...
void VideoFaceDetector::detectFacesTemplateMatching(const  cv::Mat &frame)
{
	...
	matchTemplate(frame(m_faceRoi), m_faceTemplate, m_matchingResult, TM_SQDIFF_NORMED);
